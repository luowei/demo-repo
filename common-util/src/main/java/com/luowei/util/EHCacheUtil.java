...
public abstract class EHCacheUtil {
     ...
     * 元素最大数量
     */
    public static int MAXELEMENTSINMEMORY=30000;
    /**
     * 是否把溢出数据持久化到硬盘
     */
     ...
     * 缓存的间歇时间
     */
    public static int TIMETOIDLESECONDS=86400;

    /**
     * 存活时间(默认一天)
     */
    public static int TIMETOlIVESECONDS=3*86400;
    /**
     * 需要持久化到硬盘否
     */
	...
	public static boolean modifyCache(String cacheName,
        ...
        Boolean flag = false;
		try {
			if(cacheName!=null && cacheName!="" && timeToLiveSeconds!=0L
